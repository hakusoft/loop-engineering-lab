...
export type WeatherResponse = {
  ...
  // 実 API での応答未確認の項目。取れないときは value が null になるので、
  // 表示側で必ず null を扱う。
  humidity_aloft: { value: number | null; unit: string };
  humidity_diff_ground_aloft: { value: number | null; unit: string };
  ...
  // 実 API での応答未確認の項目（soil_moisture_deep と同じ方針）。取れないときは
  // value が null になるので、表示側で必ず null を扱う。
  soil_moisture_deeper: { value: number | null; unit: string };
  // 実 API での応答未確認の項目（soil_moisture_deep / soil_moisture_deeper と同じ方針）。
  // 取れないときは value が null になるので、表示側で必ず null を扱う。
  soil_moisture_deepest: { value: number | null; unit: string };
  // 実 API での応答未確認の項目（soil_moisture_deep / soil_moisture_deeper /
  // soil_moisture_deepest と同じ方針）。取れないときは value が null になるので、
  // 表示側で必ず null を扱う。
  soil_moisture_bedrock: { value: number | null; unit: string };
  ...
  // 実 API での応答未確認の項目（PR #386 のレビュー参照）。取れないときは
  // value が null になるので、表示側で必ず null を扱う。
  precipitation_probability_mean: { value: number | null; unit: string };
  precipitation_probability_min: { value: number | null; unit: string };
