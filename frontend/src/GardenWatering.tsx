...
// 水やりしてもすぐ乾いてしまう日があるという声を受け、空気の乾きやすさの指標として
// 蒸発散量（evapotranspiration, ET0）も判定材料に加えた（Issue #270）。
const HIGH_EVAPOTRANSPIRATION_MM = 5;
...
export function gardenWateringLevel(
  ...
  if (precipitationSum < 1 && evapotranspiration >= HIGH_EVAPOTRANSPIRATION_MM) {
    return "水やり推奨";
  }
  return "普通";
}
...
export function formatGardenWatering(data: WeatherResponse): string {
  ...
  const { value: evapotranspiration } = data.evapotranspiration;
  return `庭の水やり ${gardenWateringLevel(precipitationSum, temperatureMax, humidityMin, evapotranspiration)}`;
}
