...
実際に検知・修復された例（いずれも Issue・修正 PR ともに残っている）。

| 混入したバグ | 検知経路 | Issue → 修正 PR |
|---|---|---|
| 風向 348.75°〜360° で `IndexError`（配列の添字が範囲外） | Sentry | [#43](../../issues/43) → [#44](../../pull/44) |
| 視程を存在しないキー名で読み `KeyError` | Sentry | [#67](../../issues/67) → [#68](../../pull/68) |
| 日の出・日の入りの表示が入れ替わっている | レビュー | [#61](../../issues/61) → [#62](../../pull/62) |

上 2 件は実行時に例外を投げるため Sentry が捕まえた。
3 件目は**例外を投げない**ため Sentry には出ず、フロントにテスト基盤も無い。
値が入れ替わっているだけで動いてしまうこの種のバグは、
**差分を読んで気づくしかない**。レビュー役が捕まえた一例。
