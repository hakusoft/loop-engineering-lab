...
4. CI が緑になるまで見届ける
5. 元のスレッドに Issue / PR / CI の結果を返す

Slack では役割を分けている。**依頼は人間（kaz）、返信は bot**（loop-engineering-lab アプリ。
GitHub Actions の slack-reply ワークフロー経由で投稿する）。スレッドの最後の返信が
bot なら返事待ち、人間なら続報として翌朝のループが追い対応する。

**承認とマージは自動化しない。** そこが人間の仕事。

04:30 に別のルーチンがデモ用の依頼を Slack へ投稿する（ネタは Open-Meteo の未使用パラメータと画面の小改修）。
